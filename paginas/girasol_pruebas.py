...
def girasol_pruebas():
    ...
    # Load Excel files
    excel_file_GIR_path = 'data/Seguimiento pruebas GIRASOL.xlsx'
    ...
    df_historico_gir['VOLUMEN DE AGUA'] = df_historico_gir['VOLUMEN DE AGUA'].round(1)
    ...
    with cc1:
        ...
        df_historico_gir_sorted = df_historico_gir.sort_values(by=['SARTA', 'FECHA'], ascending=[True, False])
    ...
    with c3:
        latest_dates = df_historico_gir.groupby('SARTA')['FECHA'].max().reset_index()
        latest_data = pd.merge(latest_dates, df_historico_gir, on=['SARTA', 'FECHA'])
